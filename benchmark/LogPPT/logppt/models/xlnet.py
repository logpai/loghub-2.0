...
def load_xlnet(model_path, mode="prompt-tuning"):
...
def add_label_token_xlnet(model, tokenizer, label_map):
    sorted_add_tokens = sorted(list(label_map.keys()), key=lambda x: len(x), reverse=True)
    num_tokens, _ = model.transformer.word_embedding.weight.shape
    ...
    for token in sorted_add_tokens:
        if token.startswith('i-'):
            ...
            e_token = model.transformer.word_embedding.weight.data[ws[0]]
            for i in ws[1:]:
                e_token += model.transformer.word_embedding.weight.data[i]
            e_token /= len(ws)
            model.transformer.word_embedding.weight.data[index] = e_token
